...
| [`artifactory-adopt-ci-demo`](artifactory-adopt-ci-demo/) | **The shape most customers arrive in**, as a CI pipeline. They already have the transit gateway, the VPC and the ROKS cluster, and a JFrog Artifactory they already run; `roksbnkctl` is asked only to do BNK. Nothing is created — the cluster is **registered**, the gateway attached by name, and **cert-manager adopted** (an OpenShift estate usually already has one, and `bnk up` otherwise stops on `namespaces "cert-manager" already exists`). There is **no `config.yaml` anywhere**: the whole workspace is built from `docker run -e` variables, which is the only shape an argv-only runner has. Teardown removes BNK and nothing else. |
