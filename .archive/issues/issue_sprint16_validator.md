...
**Severity**: medium
**Status**: resolved (round-3 fix; live-verified GREEN run-id `20260519-234554`; shipped in `v1.6.2`)

**Fix (round-3 — the round-2 attempt was caught broken by the live verify).** A first integration ("round-2", folded into the same `v1.6.2` cut as Issue 2 round-2) pointed terraform directly at the canonical `terraform.applied.tfvars`; live verify run-id `20260519-220236` A5 came back RED because the snapshot's PRD 07 / Sprint 11 on-disk shape is intentionally multi-section (one `# === from <source> ===` block per consumed var-file) and the *same key* legitimately appears in multiple sections — `testing_create_tgw_jumphost` is set in both the user `terraform.tfvars` and the Sprint 16 round-2 `bnk-phase-override.tfvars`, so terraform errored with `Each argument may be set only once`. The same run also showed `ibmcloud_api_key = "<redacted>"` would have overridden `TF_VAR_ibmcloud_api_key` from env (terraform precedence: var-file > env) and broken IAM auth even without the dup-key error. Round-3 closes both: new exported `config.ReadAppliedTFVarsReplayAssignments(ws, phase)` parses the snapshot into a flat key→value map (later-source-wins implicit via single-map assignment), drops `redactedVarNames`, and `internal/orchestration/applied_replay.go::LayerAppliedTFVars` writes the deduped, secret-free result to `<phase state dir>/.applied-replay.tfvars` (re-generated each lifecycle op) and returns its path. The canonical snapshot on disk stays unchanged — PRD 07's audit shape is preserved. Wiring is the same as round-2: `RunPlan` / `RunApply` / `RunTrialDown` (trial phase) and `runClusterDown` (cluster phase, via the orchestration helper to keep `cli` → `orchestration` directional); prepended as **lowest precedence** so explicit `--var-file` and the Sprint 16 round-2 `bnk-phase-override.tfvars` still win. Hermetic test `internal/orchestration/applied_replay_test.go` pins the exact regression shape (a snapshot with `testing_create_tgw_jumphost` duplicated across two sections + an `ibmcloud_api_key = "<redacted>"` line) and asserts the replay has exactly one assignment of the dup key (later-wins), no `ibmcloud_api_key` line, and lives in the phase state dir. Live A5 GREEN: `scripts/e2e-phase-handoff.sh` post-up runs bare `roksbnkctl plan -w "$WORKSPACE"` and confirms both exit 0 and the `Replaying applied tfvars from <path>` log line.

**Live verify GREEN (run-id `20260519-234554`).** Cluster phase `Apply complete! Resources: 72 added`; bnk phase `Apply complete! Resources: 60 added`; A1–A5 ✓ (incl. A5 bare `plan -w e2e-handoff` succeeded via the applied-tfvars replay); both-phase self-teardown ✓; `canada-*` residual check ✓; live recheck post-teardown cluster/VPCs/TGW/COS = 0/0/0/0. Shipped in `v1.6.2`.

**Option (b) follow-up (live-verified GREEN, run-id `20260520-035616`).** The round-3 fix closes the *snapshot-exists* case; option (b) closes the *no-snapshot* case for the same `-w <ws>` UX promise. New exported `orchestration.RequireSnapshotOrVarFile(replayed, userVarFiles, phase, verb)` returns nil whenever a snapshot is replayed OR the user passed any `--var-file`; otherwise returns a clean roksbnkctl-level error that names the phase, the verb, and the remedy (`--var-file <path/to/your/terraform.tfvars>`). Wired into `RunPlan` / `RunApply` / `RunTrialDown` (trial phase, via `in.VarFiles`) and `runClusterDown` (cluster phase — strips the `cluster-phase-override.tfvars` architectural override from the gate input so only user-supplied var-files count). The Sprint 16 round-2 `bnk-phase-override.tfvars` is not in `in.VarFiles`, so it doesn't bypass the gate either. New hermetic test `TestRequireSnapshotOrVarFile` (5 subtests) pins both branches + the message contract. New e2e assertion **A6** in `scripts/e2e-phase-handoff.sh` live-probes the option (b) gate by running bare `roksbnkctl plan -w "$WORKSPACE"` *post-init / pre-up* (no snapshot exists yet) and asserting it returns non-zero with the actionable phrase and `--var-file` remedy hint. Held *before* the GitHub Release of `v1.6.2` per the `no-piling-into-active-release` discipline (don't fold small adjacent UX fixes into a release that is mid-cycle; finish + live-verify them first).
