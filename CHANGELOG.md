...
- **Placeholders like `<name>` no longer vanish from the published book** (#239).
  Chapters 28 and 29 are generated by lifting text the generators do not control
  — Go doc comments off the `Workspace` struct, and `description` strings out of
  `terraform/variables.tf` — and interpolating it into markdown verbatim.
  Markdown passes raw HTML straight through, so a doc comment mentioning
  `~/.roksbnkctl/<name>/config.yaml` emitted a live `<name>` tag, the browser
  parsed it as an unknown element, and it rendered as **nothing**. The published
  configuration reference read "Workspace is `~/.roksbnkctl//config.yaml`" — the
  one word on the line telling you that segment is a placeholder was the word
  that disappeared. The DNS resolver docs rendered `"<ip>[:<port>]"` as `"[:]"`.

  Both generators already had an `escapePipes` helper for exactly this class of
  problem: raw text entering a markdown context that gives a character meaning.
  `|` got one because an unescaped pipe visibly wrecks a table row. `<` did not,
  because its failure mode is silent — the text simply is not there, and the
  sentence still reads like prose someone wrote that way.

  `tfvars-md` had a local partial fix, and the shape of what it missed is the
  point: it matched `<identifier>` only, so `<cluster vpc>` — a placeholder with
  a space — stayed bare and reached the published book. Both generators now share
  `tools/refgen/mdesc`, which backtick-wraps recognised placeholders and
  HTML-escapes anything else angle-bracketed, so text that is not a placeholder
  cannot open a tag either. Code spans are passed through untouched, which is
  what makes it idempotent and safe over descriptions where someone already
  worked around this by hand.

  `mdbook` had been reporting all eleven of these on every single build, and the
  warnings scrolled past. `TestGeneratedChaptersHaveNoRawHTMLTags` now asserts the
  property on the checked-in chapters, so it fails in CI instead. It is deliberately
  a check on the chapters rather than on the escaper: a generator that stopped
  calling `mdesc` altogether would pass every unit test in that package and fail
  this one.
...
configured nothing.** The cheatsheet is built from the schema itself and
published with the book; the removals are the fourth and fifth time a setting
turned out to reach a consumer that never read it, so this release also ships
