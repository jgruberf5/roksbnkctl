...
Locked decisions (do not relitigate): **in-CLI reconciler, NOT an in-cluster operator** (FLO already reconciles the CNE lifecycle in-cluster; this replaces only the terraform bootstrap); reuse `internal/k8s` dynamic SSA (no `curl`/`kubectl` shell-outs); **keep the IBM IAM trusted-profile + COS reads in terraform** (not k8s); helm strategy pinned by the architect (recommended: cert-manager static-manifest SSA + FLO/CIS via the helm Go SDK for OCI pull/templating); the **legacy terraform BNK path stays behind a flag** this sprint; `live-verify-high-issues` applies.

| Role | Scope |
|---|---|
| **Architect** | BLOCKING design inputs: (1) helm strategy + `helm.sh/helm/v3` dependency trade-off; (2) each gated CRD's exact ready-signal confirmed against the real CRDs — esp. **CNEInstance** (`k8s.f5.com/v1`) + **License** (`k8s.f5net.com/v1`) `.status`, which terraform never checks; (3) the watch-helper API + `ProgressReporter` event shape (incl. `duration`); (4) the **safe-parallelism dependency DAG** that lets staff turn serial sleeps into parallel watches (hard serial edges vs wide fan-outs, concurrency cap, watch-timeout defaults, cacheable steps). Plus the BNK-phase book chapter + a "why we left terraform local-exec" concept note. No Go. |
