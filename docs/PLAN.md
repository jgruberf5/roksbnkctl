...
## Sprint 29 — air-gap registry mirror: replicate the BNK bill-of-materials from FAR into the cluster's OpenShift internal registry (draft, not yet dispatched)

_Customer requirement (surfaced 2026-06-08): the ROKS environment forbids pulling install resources from external repositories at deploy time — every BNK artifact must be replicated into the cluster's own registry and installed from there. `roksbnkctl` pulls from FAR directly today (helm OCI + image pull-secrets, all keyed on `far_repo_url`) with no mirror path. The user framed it after the IBM COS client's CRUD shape: a tool that replicates resources defined in a specific FAR manifest into another registry._

Per [PRD 11](./prd/11-REGISTRY-MIRROR.md): a new **optional Registry phase** (after Cluster, before BNK; **not** in composite `up`, like `gateway`) and a CRUD-shaped `roksbnkctl registry {bom,replicate,list,diff,verify,prune}` command group. It reads the `f5-bigip-k8s-manifest` (already fetched for version discovery, `modules/flo/modules/flo/main.tf:417-465`) — verified during planning to be a flat, complete **81-artifact BOM** (25 charts + 56 images, tag-pinned; a pure YAML parse, no chart-rendering) — and unions the non-F5 deps `roksbnkctl` installs in place of the manifest's F5-packaged equivalents (**Jetstack cert-manager** + its quay.io images, **`bitnami/kubectl`** from Docker Hub — decision: keep + mirror, don't retarget onto the manifest's `f5-cert-manager`/`node-labeler`). It then registry-to-registry copies every chart (OCI) + image into the target — heterogeneous sources: repo.f5.com / quay.io / docker.io copy OCI→OCI, while the Jetstack classic Helm repo is pull-and-repackaged. The target is a **pluggable `RegistryTarget`** with the **OpenShift internal registry** as impl #1 (ICR / generic OCI are follow-on impls behind the interface).

The internal registry forces the abstraction to be honest about its **two faces**: the host's in-process helm provider pulls **charts** over the external **route** (`oci://<route>/<ns>/charts/…`, token auth — same shape as today's FAR OCI pull), while pods pull **images** over the in-cluster **service** (`image-registry.openshift-image-registry.svc:5000/<ns>/images/…@<digest>`) via `system:image-puller` RBAC — **no pull secret**. So the install redirect (in `internal/tf/vars.go`, driven by a `registry-mirror.json` record) **splits the single `far_repo_url`** into a chart host and an image host, and replaces `far-secret` with RBAC for image pulls. Acceptance is a **gated-live air-gap install with all egress to repo.f5.com / quay.io / docker.io blocked**, reaching a licensed, Ready TMM.

Sequencing (PRD 11 §Staged delivery): BOM builder (`internal/bnkbom`) → FAR-OCI source client + replication engine (crane / ORAS, idempotent, digest-verified) → `RegistryTarget` + OpenShift impl (route enable + push SA + pull RBAC) → install redirection → book chapter → gated-live verify. **New dependency**: go-containerregistry + ORAS — `internal/ibm/` has no container-registry client today (IAM + resource-controller + raw-REST only). Stacks on Sprint 28 (the phase model the Registry phase joins) and Sprint 27 (the `far_repo_url`-driven terraform-native chart/image refs this redirects). Replication is **client-side** (the `roksbnkctl` host bridges FAR → the registry route); an in-cluster copy job would need the cluster to reach FAR, defeating the air-gap, so it is deferred.
