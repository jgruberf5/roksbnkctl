...
## Sprint 27 — native watch-driven BNK-phase reconciler (retire terraform local-exec/curl; optimize for speed) (draft, on feature branch `sprint27-bnk-native-k8s`)

_Surfaced 2026-06-04 as an integrator architecture request. The BNK phase is, today, mostly terraform driving the cluster through `null_resource` + `local-exec` running `helm` and **raw `curl` server-side-apply**, with readiness gated by **~210s of static `time_sleep`** (cert 30 + FLO SCC 30 + FLO pods 60 + CNE CRD 30 + CNE SCC 30 + License CRD 30) plus helm `--wait` slack and `curl` retry loops — the FLO module alone is ~1,200 lines of this, and nothing watches real status (it's "apply, sleep, hope"). roksbnkctl already ships client-go v0.30 with a dynamic client + server-side apply (`internal/k8s/apply.go`), so this sprint replaces the terraform K8s interactions with a **native, watch-driven reconciler inside roksbnkctl** (`internal/bnk`)._

**Primary goal: SPEED.** The watch-based design exists to make a new BNK-version install deploy + test in a tight iteration loop. Recover the ~210s of pure sleep outright (every wait returns the instant its watched condition is true), run independent steps **concurrently** (`errgroup`, per the architect's dependency DAG), add a **fast re-deploy** path that reconciles only the delta on a version bump, drop the whole terraform init/plan/apply overhead for these modules, and report per-phase timings. The validator's gated-live benchmark must show the native path **materially faster** than terraform.

Locked decisions (do not relitigate): **in-CLI reconciler, NOT an in-cluster operator** (FLO already reconciles the CNE lifecycle in-cluster; this replaces only the terraform bootstrap); reuse `internal/k8s` dynamic SSA (no `curl`/`kubectl` shell-outs); **keep the IBM IAM trusted-profile + COS reads in terraform** (not k8s); **helm stays in terraform** — convert the three chart installs from `null_resource local-exec helm` to the `helm_release` provider (`wait = true`), **no `helm.sh/helm/v3` Go dep**, native Go replaces ONLY the `curl`-applied CRs + their `time_sleep` gates (architect draws the terraform↔Go handoff boundary); the **legacy curl-based modules stay intact as the validator's benchmark baseline** this sprint; `live-verify-high-issues` applies.

| Role | Scope |
|---|---|
| **Architect** | BLOCKING design inputs: (1) the terraform↔Go **handoff boundary** — which curl'd resources are helm prerequisites (stay terraform, e.g. namespaces + image-pull secrets) vs post-install CRs (move to Go), the no-ping-pong check, and keeping the legacy curl modules as the benchmark baseline; (2) each gated CRD's exact ready-signal confirmed against the real CRDs — esp. **CNEInstance** (`k8s.f5.com/v1`) + **License** (`k8s.f5net.com/v1`) `.status`, which terraform never checks; (3) the watch-helper API + `ProgressReporter` event shape (incl. `duration`); (4) the **safe-parallelism dependency DAG** for the Go CR layer (hard serial edges vs wide fan-outs, concurrency cap, watch-timeout defaults, cacheable steps). Plus the BNK-phase book chapter + a "why we left terraform local-exec curl" concept note. No Go. |
| **Staff** | All Go. `internal/k8s/wait.go` watch/wait primitives (CRD Established, Deployment Ready, generic CR `.status`, Job complete — actionable timeouts, no `time.Sleep` gates). `internal/bnk` reconciler porting the four terraform modules' k8s ops as apply→watch, **built for speed** (errgroup parallelism, zero fixed-sleep slack, fast-redeploy delta path with FAR OCI pull cache, warm reused clients, per-phase timing instrumentation). Orchestration seam in `RunTrialUp` + terraform gating (`deploy_bnk=false`/`deploy_cert_manager=false`), `bnk status`, native `bnk down`, the legacy `--native`/`--legacy-tf` flag. |
| **Validator** | Hermetic tests against client-go **fake** dynamic/typed clients (drive `.status` via the tracker): wait-layer timeouts/short-circuits, reconciler ordering invariants + idempotence + teardown + failure path. Gated-live `scripts/e2e-bnk-native.sh`: correctness + **the speed benchmark (native vs terraform wall-clock — native must be materially faster, asserted not just reported)** + fast-re-deploy timing. |
...
`live-verify-high-issues` applies — cluster-mutating + `up`-affecting. The integrator runs the gated-live correctness + speed benchmark (native vs terraform baseline) before merging the feature branch. The hermetic class (fake-client wait/reconciler tests) plus the gated-live driver are the closure inputs.

Version at cut: integrator-owned, **after** the feature branch proves out. New capability behind a flag → likely a minor bump; the terraform-module deletion (once native is the default) is a separate later sprint. No PRD — integrator architecture request. This is a big, interdependent migration (all four BNK k8s modules at once — they share namespaces/CRs, so a half-migration would split management across terraform + Go), so staff may land it in staged rounds (the project's round-1/round-2 pattern) gated by the live verify.

Sprint launch: `prompts/sprint27/` scaffolded alongside this PLAN.md block on the `sprint27-bnk-native-k8s` branch. Architect dispatched FIRST (its helm decision + CRD ready-signals + parallelism DAG are blocking inputs to staff); then staff; then validator after staff lands; tech-writer after integration. See `prompts/sprint27/README.md`. **Branch is not merged to main until the integrator is confident in the workings** (explicit integrator constraint).
