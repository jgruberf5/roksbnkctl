...
The prior IBM Cloud version of this tooling put both in IBM Cloud Object Storage (COS) and bound FLO's service account to an IBM Trusted Profile. `awsbnkctl` replaces both: artefacts live in S3, optionally ECR for image mirroring, and FLO's service account binds to an IAM role via IRSA (IAM Roles for Service Accounts) using the EKS OIDC provider that the EKS cluster phase creates.

> **Status:** archived (pre-pivot Terraform design). Builds on the EKS cluster phase 's `cluster_oidc_issuer_url` + `oidc_provider_arn` outputs. The S3 + IRSA design is offline-validatable. See [`docs/POST_TERRAFORM_DIRECTION.md`](../../POST_TERRAFORM_DIRECTION.md) for the current implementation direction.
...
In the prior IBM Cloud flow, (1) and (2) are uploaded to an IBM COS bucket by `the init step`; FLO pulls them at apply time using a Trusted Profile that allows COS read against the bucket. The trust chain ties the FLO Kubernetes service account → Trusted Profile → IBM IAM role with `cos:object:read` permission scoped to the bucket.

On AWS the equivalent shape:

- **S3 bucket** (server-side encrypted with KMS) — stores the FAR archive + JWT + any other module needs.
...
- An IAM OIDC provider exists for the EKS cluster (created by the EKS cluster phase; this PRD references it).
...
| Option | Verdict | Why |
|---|---|---|
| **S3 + IRSA** | **selected** | First-class AWS workload identity; no static keys on the wire; EKS-native trust chain. Matches the design intent that lifted from the prior Trusted Profile pattern. |
...
- **IAM OIDC provider** — created by the EKS cluster phase; this module references it via `data.aws_iam_openid_connect_provider`.
...
│            │                                                    │
│   ┌────────▼─────────────────┐                                  │
│   │ IAM OIDC provider        │ ← created by the EKS cluster phase            │
...
|---|---|---|---|
| `region` | string | _required_ | AWS region |
| `oidc_provider_arn` | string | _required_ | From the EKS cluster phase |
| `cluster_oidc_issuer_url` | string | _required_ | From the EKS cluster phase |
...
OBSOLETE per the post-Terraform pivot.
...
| `awsbnkctl up cluster --dry-run` | (existing) extends to include S3 + IRSA module plan. |
| `awsbnkctl doctor` | New rows: `aws s3:PutObject permission` (uses a probe key under a workspace prefix); `aws iam:GetOpenIDConnectProvider permission` probe. EC2 vCPU quota check also lands here (now that workspace + region are post-init available). |
...
- **Does the IRSA module need both `oidc_provider_arn` and `cluster_oidc_issuer_url`, or just the ARN?** → **Both.** The ARN is needed for the trust policy's `Principal.Federated`; the issuer URL is needed (with the `https://` scheme stripped) to construct the `<oidc-issuer>:sub` and `<oidc-issuer>:aud` condition keys. the EKS cluster phase exposes both.

## Resolved-in-spike

This PRD's design doesn't directly depend on earlier findings — S3, IRSA, and bucket policies are well-trodden AWS surface that `terraform validate` and mocked aws-sdk-go-v2 tests cover offline. The **live-apply path** depends on the EKS OIDC provider existing (the EKS cluster phase must apply first); the modules layer cleanly on top once that prerequisite is in place.

IRSA + S3 are independent of the data-plane choice, so any changes to the EKS node shape do not affect this PRD's design.

## Cross-references

- The project README — release plan.
...
- [`POST_TERRAFORM_DIRECTION.md`](../../POST_TERRAFORM_DIRECTION.md) — project-wide post-pivot migration context.