...
// TestSeedVarFileInterview_SetsSanitizedPrefix — the --var-file path derives
// a sanitized Prefix (preferring the file's cluster name) and defaults every
// resource to create, so the generated base is collision-safe while the
// operator's terraform.tfvars.user still overrides it via layering.
func TestSeedVarFileInterview_SetsSanitizedPrefix(t *testing.T) {
	seeds := varFileSeeds{
		HasCreateCluster: true, CreateCluster: true,
		HasClusterName: true, ClusterName: "My_Cluster.1",
		HasOCPVersion: true, OCPVersion: "4.18",
		HasWorkersPerZone: true, WorkersPerZone: 2,
	}
	cluster, prefix, resources, err := seedVarFileInterview(seeds, true, "fallback", "4.18", 1, "demo-ws")
	if err != nil {
		t.Fatalf("seedVarFileInterview: %v", err)
	}

	// Prefix prefers the file's cluster name, sanitized.
	want := naming.SanitizeToPrefix("My_Cluster.1")
	if prefix != want {
		t.Errorf("prefix = %q; want sanitized cluster name %q", prefix, want)
	}
	if err := naming.ValidatePrefix(prefix); err != nil {
		t.Errorf("derived var-file prefix %q does not validate: %v", prefix, err)
	}
	if !cluster.Create || cluster.Name != "My_Cluster.1" {
		t.Errorf("cluster = %+v; want Create=true Name=My_Cluster.1 (seed-driven)", cluster)
	}
	// All-create default so the generated base is collision-safe.
	if resources == nil || !resources.TransitGateway.Create || !resources.RegistryCOS.Create {
		t.Errorf("resources must default to all-create; got %+v", resources)
	}
}

// TestSeedVarFileInterview_PrefixFallsBackToWorkspaceName — when the file
// does NOT carry a cluster name (only the default cluster name applies), the
// prefix seed falls back to the workspace name. The fallback is keyed on
// seeds.HasClusterName (a file-supplied name), not on the resolved default
// cluster name, so a defaulted cluster name does NOT seed the prefix.
func TestSeedVarFileInterview_PrefixFallsBackToWorkspaceName(t *testing.T) {
	seeds := varFileSeeds{HasCreateCluster: true, CreateCluster: true}
	_, prefix, _, err := seedVarFileInterview(seeds, true, "the-default-cluster", "4.18", 1, "ws-name-fallback")
	if err != nil {
		t.Fatalf("seedVarFileInterview: %v", err)
	}
	// No file-supplied cluster name ⇒ seed from the workspace name.
	want := naming.SanitizeToPrefix("ws-name-fallback")
	if prefix != want {
		t.Errorf("prefix = %q; want %q (from workspace name, no file cluster name)", prefix, want)
	}
}

// TestSeedVarFileInterview_DeclinedClusterCapturesExisting — declining the
// cluster create (via seeds) captures the existing cluster name into
// Cluster.Name (which doubles as the existing id/name when Create=false).
func TestSeedVarFileInterview_DeclinedClusterCapturesExisting(t *testing.T) {
	seeds := varFileSeeds{
		HasCreateCluster: true, CreateCluster: false,
		HasClusterName: true, ClusterName: "existing-roks",
	}
	cluster, _, _, err := seedVarFileInterview(seeds, false, "ignored", "4.18", 1, "demo-ws")
	if err != nil {
		t.Fatalf("seedVarFileInterview declined cluster: %v", err)
	}
	if cluster.Create {
		t.Errorf("cluster.Create = true; want false (seed declined)")
	}
	if cluster.Name != "existing-roks" {
		t.Errorf("cluster.Name = %q; want the existing name %q", cluster.Name, "existing-roks")
	}
}
