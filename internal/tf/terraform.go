...
// source), the per-roksctl-workspace state directory, and a configured
...
//     The key is never written to disk by roksctl.
//
...
func Open(
	...
	if wsCfg == nil {
		return nil, fmt.Errorf("workspace config is nil (run `roksctl init`)")
	}
	...
	backendOverride := filepath.Join(sourceDir, "roksctl_backend_override.tf")
	backendBody := fmt.Sprintf(`# Generated by roksctl. Do not edit by hand.
...
`, filepath.Join(stateDir, "terraform.tfstate"))
	...
	// terraform-exec inherits the roksctl process env when SetEnv is
	...
	// in argv / `ps` output).
	//
	// Setting on the roksctl process env is acceptable because roksctl
...
// StateDir is the roksctl per-workspace state root.
...
// config.yaml). Optional — if present, roksctl passes it to terraform
...
// roksctl's RenderTFVars doesn't expose (testing_*, roks_min_worker_*,
...
// (excluding api_key — see WriteTFVars in vars.go). roksctl-managed
...
// name as a subdir; roksctl pre-creates them in Open.
...
// override land cleanly even if a previous roksctl run initialised
...
// terraform-exec doesn't allow interactive prompts; roksctl's own
