...
func TestRenderTFVars_LegacySparse_ByteUnchanged(t *testing.T) {
	...
	// bnk_line is the ONE addition to this baseline since it was pinned, and it
	// is semantically neutral: this workspace names no manifest version, so the
	// line derives from DefaultManifestVersion and equals terraform's own
	// default. Nothing about the plan changes.
	//
	// It is rendered anyway, on this legacy path too, for the reason
	// cluster_network_mode is: the tool default and the terraform default must
	// never be ABLE to disagree. They agree today by coincidence of both being
	// 2.3 — the day DefaultManifestVersion moves to 2.4, a workspace that never
	// mentioned BNK would otherwise derive 2.4 and silently plan 2.3.
...
func TestRenderTFVars_LegacyAttach_ByteUnchanged(t *testing.T) {
	...
	// bnk_line: see the note on the create-branch baseline above. Same neutral
	// addition, same reason.
	const want = `# Generated by roksbnkctl. Do not edit by hand — run ` + "`roksbnkctl init`" + ` to update.
# IBMCLOUD_API_KEY is NOT written here; it's passed via TF_VAR env var.
