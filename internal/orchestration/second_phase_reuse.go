...
func writeBnkFLPOverride(stateDir, workspace string, ws *config.Workspace) (string, error) {
	var endpoint, caB64, source string
...
func writeAndInitSecondPhase(ctx context.Context, tfws *tf.Workspace, ws *config.Workspace, workspace string, destroy bool, w io.Writer) ([]string, error) {
	...
	if ws.BNK.LicenseMode == "f5licenseproxy" {
		flpOverride, ferr := writeBnkFLPOverride(tfws.StateDir(), workspace, ws)
		switch {
