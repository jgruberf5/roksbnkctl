...
// the endpoint + decoded root-CA PEM as forced tfvars for `bnk up` in
// f5licenseproxy mode. A missing handoff is an actionable error (run `flp up`).
func writeBnkFLPOverride(stateDir, workspace string) (string, error) {
	out, err := config.ReadFLPOutputs(workspace)
	if err != nil {
		return "", fmt.Errorf("license_mode is f5licenseproxy but the F5 License Proxy handoff is missing: %w", err)
	}
	...
	pem, derr := base64.StdEncoding.DecodeString(out.RootCAB64)
	if derr != nil {
		return "", fmt.Errorf("decoding FLP root CA from flp-outputs.json: %w", derr)
	}
	overridePath := filepath.Join(stateDir, bnkFLPOverrideFile)
	content := fmt.Sprintf(`# Generated by roksbnkctl. Do not edit by hand.
# FLP-licensing handoff for `+"`bnk up`"+` (from flp-outputs.json). Points the BNK
# License CR at the in-cluster F5 License Proxy. Forced — wins over config.yaml.
flp_license_server_url = %q
license_server_root_ca = %q
`, out.Endpoint, string(pem))
...
func writeFLPPhaseOverride(tfws *tf.Workspace, co *config.ClusterOutputs) (string, error) {
	return writeFLPPhaseOverrideAt(tfws.StateDir(), co)
}
...
func writeFLPPhaseOverrideAt(stateDir string, co *config.ClusterOutputs) (string, error) {
...
# phase's creation is forced OFF and deploy_flp ON. Forced — wins over
