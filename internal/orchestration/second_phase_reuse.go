...
func writeFLPPhaseOverride(tfws *tf.Workspace, co *config.ClusterOutputs, vsiMode bool) (string, error) {
	return writeFLPPhaseOverrideAt(tfws.StateDir(), co, vsiMode)
}
...
func writeFLPPhaseOverrideAt(stateDir string, co *config.ClusterOutputs, vsiMode bool) (string, error) {
...
deploy_flp = %v
deploy_flp_vsi = %v
`, clusterIdentity(co), co.VPCID, !vsiMode, vsiMode)
	if err := os.WriteFile(overridePath, []byte(content), 0o644); err != nil {
