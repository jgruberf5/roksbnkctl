...
// (use_existing_cluster_vpc + existing_cluster_vpc_id + cluster_vpc_id +
// roks_cluster_id_or_name + testing_transit_gateway_name). It does NOT pin
...
//
// testing_transit_gateway_name is emitted only when the handoff carries a
// non-empty name; otherwise the line is omitted and the normal config.yaml
// render supplies it (a `cluster register` without a TGW output).
func writeTestingPhaseOverrideAt(stateDir string, co *config.ClusterOutputs) (string, error) {
	...
	if co.TransitGatewayName != "" {
		tgwLine = fmt.Sprintf("testing_transit_gateway_name = %q\n", co.TransitGatewayName)
	}
	content := fmt.Sprintf(`# Generated by roksbnkctl. Do not edit by hand.
...
deploy_bnk = false
deploy_cert_manager = false
%s`, clusterIdentity(co), co.VPCID, co.VPCID, tgwLine)
	if err := os.WriteFile(overridePath, []byte(content), 0o644); err != nil {
...
testing_create_client_vpc = false
deploy_gateway = true
`, clusterIdentity(co), co.VPCID, co.VPCID)
	if err := os.WriteFile(overridePath, []byte(content), 0o644); err != nil {
