...
// Second-phase cluster-shared-infra handoff.
//
// The `up` flow applies the SAME root
...
//       2026-05-27 live evidence shows
...
//       carry-over from an older workspace) that might import the
...
func writeBnkPhaseOverrideAt(stateDir string, co *config.ClusterOutputs) (string, error) {
	...
	content := fmt.Sprintf(`# Generated by roksbnkctl. Do not edit by hand.
# Second/bnk-phase override. The Sprint 23 phase-separation-leak
# follow-up added the explicit create_roks_registry_cos_instance gate.
# Sprint 27 REVERSED the Sprint 23
...
func writeFLPPhaseOverrideAt(stateDir string, co *config.ClusterOutputs, vsiMode bool, clusterAbsent bool) (string, error) {
	...
	// A VPC to adopt, or none — the standalone create_vpc path (#76) has no id
	// yet because terraform is about to build one. Forcing
	// use_existing_cluster_vpc = true with an empty id fails at plan, which is
	// what made create_vpc unusable for a cluster-less proxy.
	vpcLines := "use_existing_cluster_vpc = false\n"
	if co.VPCID != "" {
		vpcLines = fmt.Sprintf("use_existing_cluster_vpc = true\nexisting_cluster_vpc_id = %q\n", co.VPCID)
	}

	// The header describes whichever path actually produced the file. It used to
	// assert "the cluster already exists" unconditionally, which is false on the
	// standalone path — where cluster_absent = true says the opposite three lines
	// below. A generated file that contradicts itself is read as a bug in the
	// tool by whoever opens it.
	header := "# FLP-phase override. The cluster already exists; this phase installs ONLY\n" +
		"# the F5 License Proxy, reusing the cluster from cluster-outputs.json."
	if clusterAbsent {
		header = "# FLP-phase override (STANDALONE). There is NO cluster; this phase deploys\n" +
			"# ONLY the F5 License Proxy as a VSI."
	}
...
create_roks_cluster = false
roks_cluster_id_or_name = %q
%screate_roks_transit_gateway = false
create_roks_registry_cos_instance = false
deploy_bnk = false
deploy_cert_manager = false
testing_create_tgw_jumphost = false
...
deploy_flp_vsi = %v
cluster_absent = %v
`, header, identity, vpcLines, !vsiMode, vsiMode, clusterAbsent)
	if err := os.WriteFile(overridePath, []byte(content), 0o644); err != nil {
