...
> **Status:** v0.9 release candidate. Four execution backends (local, docker, k8s, ssh), GSLB-aware DNS probe, and terraform-via-docker all wired and tested. Real-cluster shake-out items are tracked in [docs/SHAKEOUT.md](docs/SHAKEOUT.md). See [CHANGELOG.md](CHANGELOG.md) for the full release surface.
...
- **`--backend docker` (v0.9)** — run wrapped tools (`ibmcloud`, `iperf3`) inside vendored container images instead of as host processes. Frozen toolchain version, no host install required. Credentials propagated by reference — `IBMCLOUD_API_KEY` value never appears in `docker inspect`. Per-tool defaults via workspace `config.yaml`'s `exec:` block. See [chapter 17](https://jgruberf5.github.io/roksbnkctl/book/17-execution-backends.html).
- **`--backend k8s` + `--backend ssh` (v0.9)** — k8s backend uses an in-cluster ops pod for ad-hoc commands and one-shot Jobs for ephemeral runs (iperf3 client, future terraform); SSH backend wraps Sprint 1's `--on` SSH client with file materialization, env propagation (SetEnv + wrapper fallback), and Ubuntu apt-bootstrap behind `--bootstrap`. See [chapter 17](https://jgruberf5.github.io/roksbnkctl/book/17-execution-backends.html) for backend mechanics, [chapter 18](https://jgruberf5.github.io/roksbnkctl/book/18-choosing-backend.html) for which backend to use per scenario, and [chapter 19](https://jgruberf5.github.io/roksbnkctl/book/19-in-cluster-ops-pod.html) for the in-cluster ops pod lifecycle.
- **GSLB-aware DNS probe + terraform via docker (v0.9)** — `roksbnkctl test dns --gslb-compare` fans the probe across local + in-cluster + SSH-target vantages and emits a `gslb_divergence` boolean for CI assertions; `--require-divergence` is the exit-code-keyed form of the same assertion. `roksbnkctl up`/`plan`/`apply`/`destroy --backend docker` runs terraform inside `hashicorp/terraform:1.5.7` with the workspace's state directory bind-mounted, host-user-owned. See [chapter 21](https://jgruberf5.github.io/roksbnkctl/book/21-dns-testing-gslb.html) for the DNS probe + GSLB scenarios, [chapter 22](https://jgruberf5.github.io/roksbnkctl/book/22-throughput-testing.html) for throughput, and [chapter 17](https://jgruberf5.github.io/roksbnkctl/book/17-execution-backends.html) §"terraform via docker" for the terraform-docker mechanics.
