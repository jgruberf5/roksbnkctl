...
- `openshift_cluster_crn`: CRN of the OpenShift cluster — used to link the trusted profile to a ROKS service account in `flo_namespace` (sourced from cluster data source). The account is `var.trusted_profile_sa_name`; **empty (the default) derives `f5-cne-controller-<flo_namespace>-f5-cne-controller-serviceaccount`**, which is what FLO creates. The variable exists so an estate that can make FLO name the account differently can say so — it does not rename anything by itself
