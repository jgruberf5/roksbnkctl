...
## Round-3 Closure (Issue 2 — perf, 2026-05-20)

Round-2 (commit `5a1b13b`) shipped a sound architectural fix for the
COS client setup; the round-2 hermetic suite pinned the
single-Client-per-invocation + shared-IAM-creds + per-region
S3-handle-cache invariants and stayed green. **Live verify caught a
residual.** `roksbnkctl cos object list bnk-schematics-resources
--instance bnk-orchestration` took **89s** wall-clock vs **1.4s** for
`ibmcloud cos objects` on the same bucket / same network — **~63× of
Issue 2 AC #1's ≤2× target**. The cost was in
`DefaultBucketRegionResolver`'s **HeadBucket-probe sweep across a fixed
fallback region list**: each missed candidate paid a full round-trip
plus the SDK's default retry/backoff before the resolver landed on the
bucket's actual region. Round-3 collapses that N-call sweep into a
...
bucket's true region — the op is instance-scoped via IAM, not
bucket-region-scoped).

### SDK call wired

`(*s3.S3).GetBucketLocationWithContext(ctx,
&s3.GetBucketLocationInput{Bucket: aws.String(bucket)})` — invoked on
`c.s3` (the home-region handle the round-2 Client already constructs).
No new sessions, no new credentials, no new regional handles built
during the resolve step. NoSuchBucket is detected via
`awserr.Error.Code() == s3.ErrCodeNoSuchBucket` and wrapped into an
error naming both the bucket AND the instance CRN so the operator can
distinguish "wrong bucket name" from "wrong `--instance`" (Issue 3 AC
#3's clarity goal stays satisfied — and is now pinned by a fresh
hermetic test in addition to the round-2 resolver-error test).

### LocationConstraint parsing rule

IBM COS returns LocationConstraint payloads in five shapes:

```
us-south-standard   // canonical region + default storage class
us-south-smart      // + smart-tier
us-south-vault      // + vault-tier
us-south-cold       // + cold-tier
us-south            // bare region (older buckets / cross-region)
```

The parser (`parseLocationConstraint`) trims whitespace defensively
and strips a trailing `-{standard,smart,vault,cold}` suffix if
present; bare region strings pass through unchanged; empty payload
returns `""` so the resolver can fall back to the Client's home region
rather than emit a blank endpoint downstream.

### Code changes

- `internal/cos/client.go` — replaced **only**
  `DefaultBucketRegionResolver`'s body (HeadBucket-probe sweep →
  GetBucketLocation single-call) + added `parseLocationConstraint`
  helper + `locationStorageClassSuffixes` table. The old
  `probeBucketRegion` + `fallbackRegions` are gone (they were
  exclusively the round-2 resolver's implementation detail). The
  `BucketRegionResolver` type, `regionFor` cache, `regionalS3` map,
  `Client` struct, and `s3ForBucket` are byte-identical to round-2.
  Imports gain `aws/awserr` (used for the NoSuchBucket type-assert).
- `internal/cos/client_default_resolver_test.go` — NEW additive test
  file (round-1 / round-2 parity discipline preserved; no edits to
  any pre-existing `_test.go`). Four hermetic invariants:
  1. `TestDefaultResolver_GetBucketLocation_CalledOncePerBucket` —
     spins an `httptest.Server`, points the Client's home handle at
     it, drives 20 `s3ForBucket` calls on the same bucket, asserts the
     server saw exactly 1 GetBucketLocation hit (round-2's
     `bucketRegions` cache + round-3's single-call resolver compose).
  2. `TestDefaultResolver_ParseLocationConstraint_StripsStorageClassSuffix`
     — table-driven across all four storage-class suffixes + bare
     regions + whitespace + empty payload.
  3. `TestDefaultResolver_NoSuchBucket_ErrorNamesBucket` — fake server
     returns 404 + NoSuchBucket XML; asserts the error includes both
     the bucket name AND the instance CRN.
  4. `TestDefaultResolver_BareRegionConstraint_PassesThrough` — bare
     `eu-de` LocationConstraint round-trips through as `eu-de`.

### Files touched

Exactly two:

- `internal/cos/client.go` (modified — resolver body + new helper).
- `internal/cos/client_default_resolver_test.go` (new — 4 tests).

Round-1 / round-2 `_test.go` files: ZERO edits (`git status --short |
grep _test.go` shows only the one new file).

### Gates run (sandbox-allowed)

- `go build ./...` → clean (exit 0).
- `go vet ./...` → clean (exit 0).
- `gofmt -l internal/` → empty (exit 0).
- `go test -race -v ./internal/cos/` → **ok 2.155s, all tests pass**:
  - 4 new round-3 tests (this file).
  - 3 round-2 perf tests (`client_perf_test.go`) — unchanged, green.
  - 4 round-2 region tests (`client_region_test.go`) — unchanged,
    green.
  - All Issue 1 `bucket_get_test.go` + `bucket_default_test.go`
    tests — unchanged, green.

### Acceptance criteria by name

Issue 2:
1. **AC #1 wall-clock ≤ 2× `ibmcloud cos`** — **predicted green**.
   Replacing an N-region HeadBucket sweep (~80s) with a single
   GetBucketLocation call (~1s) drops the resolver's wall-clock
   contribution by ~80s. Round-2 already pinned that the rest of the
   path (Client construction, IAM token reuse, regional handle cache)
   is single-fetch; the resolver was the only remaining N-call site.
   Predicted live wall-clock for the bnk-schematics-resources sample:
   **~2–4s** (1.4s `ibmcloud` baseline + 1 GetBucketLocation + 1 IAM
   exchange + 1 ListObjects). Integrator runs the live `!` verify.
2. **AC #2 single-construction-per-invocation** — ✓ (round-2 invariant
   unchanged; round-2 perf test still green).
3. **AC #3 benchmark micro-test** — fulfilled by the call-count
   hermetic test (1 GetBucketLocation across 20 same-bucket lookups)
   + the round-2 single-construction + regional-cache + shared-creds
   invariants. The live verify is still the real perf gate.

Issue 3:
1. **AC #1 cos object list returns the real listing** — predicted
   green once AC #1 above lands (the round-2 us-south endpoint
   routing is the same; round-3 just makes the region lookup fast).
2. **AC #2 fake-resolver test pins us-south endpoint** — ✓ (round-2
   test unchanged, still green).
3. **AC #3 error names the bucket on lookup failure** — ✓ (round-2
   test still green AND new NoSuchBucket test pins it for the
   GetBucketLocation path).

### Predicted live wall-clock

`roksbnkctl cos object list bnk-schematics-resources --instance
bnk-orchestration`:

- Pre-round-3: ~89s (HeadBucket sweep across ~8 fallback regions, each
  with SDK retry/backoff).
- Post-round-3 prediction: **~2–4s** wall-clock, comfortably inside
  Issue 2 AC #1's ≤2× target relative to `ibmcloud cos objects`'s
  1.4s baseline.

### Status

Issue 2 **integrated hermetically green**; stays `open` until the
integrator's fresh live `!` verify reads `≤ ~3s` against
`bnk-schematics-resources`. Issue 3 invariants unchanged from round-2;
its live verify rides on the same integrator pass.
