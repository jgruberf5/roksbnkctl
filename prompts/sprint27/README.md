...
5. **Helm strategy is architect's to pin** before staff codes the chart installs (recommended: cert-manager via static manifest + SSA; FLO/CIS via the helm Go SDK for OCI pull + runtime templating). `helm.sh/helm/v3` is a heavy dep — don't add it before the decision lands.
...
| Role | Scope |
|---|---|
| **Architect** | BLOCKING design inputs: (1) helm strategy + dependency trade-off; (2) each gated CRD's exact ready-signal, confirmed against real CRDs — esp. CNEInstance + License (`.status` shape); (3) watch-helper API + `ProgressReporter` event shape (incl. `duration`); (4/Issue 5) the safe-parallelism dependency DAG that lets staff turn serial sleeps into parallel watches. Plus the BNK-phase book chapter + a "why we left terraform local-exec" concept note. No Go. |
