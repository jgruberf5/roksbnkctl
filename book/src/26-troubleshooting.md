...
**Root cause**: a `helm_release` whose `repository` is a bare chart-repo URL (cert-manager off `https://charts.jetstack.io`) resolves through Helm's anonymous-repo path, which downloads the repo index into `helmpath.CachePath("repository")` — i.e. `$HELM_CACHE_HOME/repository`, defaulting to `$HOME/.cache/helm/repository`. Crucially, that path is governed by **`HELM_CACHE_HOME`**, *not* `HELM_REPOSITORY_CACHE` (which only covers the named-repo / `helm repo` CLI path). In a runner whose `$HOME` resolves to an empty or non-writable path — how the bnk-forge tools-runner image launches the phases — that directory can't be created, so the download fails even though `charts.jetstack.io` is perfectly reachable (the index downloads on the fly once the dir is writable — no pre-`helm repo add` is needed). The sibling symptom is a non-fatal `warning: creating <HOME>/.kube: mkdir <HOME>: permission denied` from the post-apply admin-kubeconfig fetch.

**Fix**: `roksbnkctl` exports writable, `$ROKSBNKCTL_HOME`-relative Helm homes before invoking terraform/helm — `HELM_CACHE_HOME`, `HELM_CONFIG_HOME`, `HELM_DATA_HOME` (plus `HELM_REPOSITORY_CACHE` / `HELM_REPOSITORY_CONFIG` / `HELM_REGISTRY_CONFIG`) under `$ROKSBNKCTL_HOME/.helm/`, and `KUBECONFIG` at `$ROKSBNKCTL_HOME/.kube/config` when `$HOME/.kube` isn't writable — and pre-creates those dirs (see [`prepareToolEnv` in `internal/tf/terraform.go`](https://github.com/jgruberf5/roksbnkctl/blob/main/internal/tf/terraform.go)). The terraform helm provider runs as a plugin subprocess that inherits this env, so the redirect reaches the chart download. `$ROKSBNKCTL_HOME` is the persisted workspace tree (`/work/.roksbnkctl` under bnk-forge), so the cache is writable and survives across the phased invocations. Each var is set only when unset, so you can still point the cache at an air-gap mirror by exporting your own value. Set `ROKSBNKCTL_DEBUG=1` to log the resolved `HELM_CACHE_HOME` / `KUBECONFIG` before each phase's terraform exec. No action needed on a normal host run — `$HOME` there is already writable.
