...
| `cneinstance_tmm_k8s_routes` | `172.17.0.0/18` | Pod CIDR TMM routes to (`TMM_K8S_ROUTES`). `bnk.network.tmm_k8s_routes`. Set to your cluster's pod subnet if non-default. **2.3 only** — 2.4 uses `ENABLE_K8S_ROUTES` and this never reaches the CNEInstance (#307). |
