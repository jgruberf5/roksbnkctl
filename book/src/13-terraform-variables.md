...
| You have a complete `./terraform.tfvars` you want this workspace to always use | `roksbnkctl init -w <ws> --var-file ./terraform.tfvars` — drops the file to both phase state dirs as `terraform.tfvars.user` so bare `-w <ws>` commands Just Work. See [Chapter 6 §"Skip the interview: `init --var-file`"](./06-workspaces.md#skip-the-interview-init---var-file). |
