...
- **v0.9 — four execution backends.** Every external tool (`ibmcloud`, `iperf3`, `terraform`) becomes selectable across `local | docker | k8s | ssh` backends via `--backend`. You'll be able to run iperf3 entirely in-cluster without installing it on the host, run ibmcloud in a Docker container with a pinned version, or proxy any tool through a jumphost. [Chapter 17](./17-execution-backends.md) covers the design.
