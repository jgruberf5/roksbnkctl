...
Verify with `roksbnkctl targets list` — you should see `jumphost` plus one `jumphost-<zone>` per AZ. Each is a first-class `--on` target (full `kubectl`/`oc`/`ibmcloud`/`shell` passthrough, no SSH hop): `roksbnkctl --on jumphost-ca-tor-2 kubectl get pods`.

> **Preinstalled load generators.** Every jumphost — the TGW `jumphost` and each per-AZ `jumphost-<zone>` — comes with **`iperf3` and `h2load`** (the `nghttp2-client` package) already installed by the Testing-phase `user_data`. So both [`test throughput`](./22-throughput-testing.md) (iperf3) and the [performance matrix](./22a-performance-matrix.md) (iperf3 + h2load) run over `--backend ssh:<target>` with **no `--bootstrap`** — the SSH backend's apt-bootstrap path ([Chapter 17 §"SSH backend"](./17-execution-backends.md#ssh-backend)) is needed only for hosts you registered by hand. Like the singular `jumphost`, registration is **best-effort and idempotent** — a parse/write failure logs a single `warning:` and does not fail `up`, and re-running `up` after a floating-IP rotation refreshes each `jumphost-<zone>` host in place. When `testing_create_cluster_jumphosts = false` (or the output is absent/empty), this is a silent no-op — only `jumphost` is seeded, with no warning noise.
