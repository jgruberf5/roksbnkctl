...
| `trusted_profile.service_account` | string | `f5-cne-controller` | Account allowed to assume the CNE controller's IBM Cloud Trusted Profile. Safe to share across clusters — uniqueness comes from the profile name and the link's cluster CRN. Env: `ROKSBNKCTL_TRUSTED_PROFILE_SA`. |
