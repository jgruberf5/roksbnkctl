...
**`trusted_profile.service_account` is a matcher, not a pointer — and the name differs by BNK line.** The IBM Cloud Trusted Profile is the identity the CNE controller assumes to manage the VPC network attachments it creates for TMM, so it works without a stored API key. IBM's trust relationship compares a pod's service-account token against `crn` / `namespace` / `name` with `EQUALS`. Left empty, `roksbnkctl` derives the name the controller actually runs as, and that is **not the same name on both lines**:

| Line | Derived service account |
| --- | --- |
| 2.3 | `f5-cne-controller-<flo_namespace>-f5-cne-controller-serviceaccount` |
| 2.4 | `f5-cne-controller` |

On 2.3 it is FLO's helm construction, `<release>-<chart>-serviceaccount` with the namespace baked into the release name. On 2.4 the CNE controller chart names the account outright. Both names were read off live installs rather than inferred.

Write a name that matches neither and nothing can assume the profile. **The install still reports itself healthy.** The controller logs the token exchange failing (`BXNIM0398E … no matching rule or link found`), then falls back to `Cloud environment is not IBM or cloud provider instance is nil, skipping cloud address prefix handling` and skips VPC address-prefix and route programming altogether — so nothing reaches the listener addresses and egress SNAT replies have no route back. Meanwhile `Infra`, `GatewaySettings`, `Gateway` and `EgressGateway` all report `Programmed=True`, because those conditions are computed without ever calling the cloud, and the profile looks correct in the IBM Cloud console the whole time. **A status check cannot detect this; only traffic can** (`roksbnkctl testing` exercises exactly that path).
