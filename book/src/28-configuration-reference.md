...
| `bnk.network.vlan_prefixlen` | int | `24` | TMM self-IP prefix length (F5SPKVlan `spec.prefixlen_v4`) → `cneinstance_vlan_prefixlen`. Normally match your VLAN CIDRs. `roksbnkctl init` asks for it **after** the zone CIDRs and suggests the mask they share, when all of them agree — but that is a prompt default and nothing more. Override it freely: a mask that deliberately disagrees with its subnet changes what TMM treats as directly connected, which is how static routes are used to force a specific traffic pattern. Nothing validates the two against each other. |
