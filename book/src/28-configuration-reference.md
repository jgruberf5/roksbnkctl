...
| `trusted_profile.service_account` | string | `f5-cne-controller` | a Kubernetes service account name | The account allowed to **assume** the CNE controller's IBM Cloud Trusted Profile → `flo_trusted_profile_sa_name`. Must match the account the controller pod actually runs as — the FLO Helm chart creates it, not this tool, and a mismatch produces a profile nobody can assume (the symptom is an IBM Cloud authorization failure at VPC-attachment time, naming neither this setting nor the profile). **Safe to share across clusters:** uniqueness comes from the profile name, which carries the cluster name, and from the link's cluster CRN. The same value also drives the privileged-SCC binding for that account, so the two cannot drift. Env: `ROKSBNKCTL_TRUSTED_PROFILE_SA`. |
