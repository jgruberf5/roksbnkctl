...
| `flo_utils_namespace` | string | `f5-utils` | RFC 1123 namespace label | Namespace for the F5 utility components (`flo_utils_namespace`). **Set this and `flo_namespace` to the SAME value to install into one namespace instead of two** — verified end to end against BNK 2.3 (30 pods in the shared namespace, no second namespace created), on both a fresh install and a re-install onto an existing cluster. The utils-side namespace and its duplicate secrets are simply not created. Env: `ROKSBNKCTL_FLO_UTILS_NAMESPACE`. |
...
| `bnk.flo_utils_namespace` | string | `f5-utils` | F5 utility-components namespace → `flo_utils_namespace`. Equal to `bnk.flo_namespace` ⇒ one shared namespace (verified against BNK 2.3). Env: `ROKSBNKCTL_FLO_UTILS_NAMESPACE`. |
...
| `bnk.flp.vsi.name_prefix` | string | (empty) | Prefixes the FLP VSI's IBM Cloud resource names (instance, subnet, security group, floating IP, boot volume, and its VPC when created). **Empty keeps the legacy unprefixed names**, because renaming a terraform resource *replaces* it — so an existing proxy is not destroyed and rebuilt on upgrade. Set it on a new deployment for either of two things the literals prevented: more than one standalone FLP per account (they otherwise all collide on `flp-vsi`), and visibility to [`cleanup`](./11-tearing-down.md), which sweeps by `<prefix>-*` and cannot see unprefixed proxy resources. |
