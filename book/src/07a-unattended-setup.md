...
| `ROKSBNKCTL_VLAN_PREFIXLEN` | `bnk.network.vlan_prefixlen` | TMM self-IP prefix length (`F5SPKVlan spec.prefixlen_v4`). **Independent of the zone CIDRs and not derived from them** — a mask that deliberately disagrees with its subnet, paired with static routes, is how a specific traffic pattern is forced. Settable on its own — the zones do not have to be restated. |
